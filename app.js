(function () {
  ...
  function renderThirds() {
    ...
    if (thirds.length === 0) {
      thirdsTableBody.innerHTML = '<tr><td colspan="5" class="loading-row">No data available.</td></tr>';
      return;
    }
    ...
    thirds.forEach((team, i) => {
      ...
      const ptsGdGf = team.points != null
        ? `${team.points} <span class="third-gd">${team.gd >= 0 ? '+' : ''}${team.gd}</span> <span class="third-gf">(${team.gf})</span>`
        : '—';
      const fifaCell = team.fifaRank != null ? '#' + team.fifaRank : '—';
      ...
      rows += `<tr data-team="${team.name}" class="${team.qualifies ? 'third-qualifies' : 'third-eliminated'}">
        ...
        <td class="col-num third-ptsgd" title="${team.points != null ? `${team.points} point${team.points === 1 ? '' : 's'}, goal difference ${team.gd >= 0 ? '+' : ''}${team.gd}, ${team.gf} scored` : ''}">${ptsGdGf}</td>
        <td class="col-num" title="Official FIFA World Ranking">${fifaCell}</td>
